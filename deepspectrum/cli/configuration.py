...
GENERAL_OPTIONS = [
    click.argument('input', type=click.Path(dir_okay=True, file_okay=True, exists=True, readable=True)),
    click.option('-c', '--config', type=click.Path(readable=True, dir_okay=False),
                 help=
    ...
    click.option('-np', '--number-of-processes',
...
PLOTTING_OPTIONS = [
    ...
    click.option(
        '-nm', '--number-of-melbands',
        type=int, callback=_check_positive,
    ...
    click.option(
        ...
        default=None),
    click.option('-ppdfs', '--pretty_pdfs', is_flag=True,
...
WRITER_OPTIONS = [
    ...
    click.option(
        ...
        required=True, type=click.Path(writable=True, dir_okay=False)),
    click.option(
        '-r', '--reduced',
    ...
    click.option('-tc', '--time-continuous', is_flag=True,
...
LABEL_OPTIONS = [
    ...
    click.option(
        ...
        default=None, type=click.Path(exists=True, dir_okay=False, readable=True)),
    click.option(
        '-el', '--explicit-label',
        nargs=1,
...
class Configuration:
    ...
    def __init__(self, plotting=True, extraction=True, writer=True,
                 file_type='wav', input=None, config='deep.conf', number_of_processes=max_np, colour_map='viridis',
                 mode='mel', frequency_scale='linear', delta=None, frequency_limit=None, nfft=None, start=0, end=None,
        ...
        if self.plotting:
            ...
            self.plotting_args['output_spectrograms'] = abspath(
                spectrogram_out) if spectrogram_out is not None else None
            self.plotting_args['output_wavs'] = abspath(
                wav_out) if wav_out is not None else None
            if pretty_pdfs:
    ...
    def _read_label_file(self):
        ...
        if missing_labels:
            log.info(f'No labels for: {len(missing_labels)} files. Only processing files with labels.')
        ...
        if isfile(self.config):
            ...
            if self.extraction:
                ...
                if self.backend == 'keras':
                    log.info('Using keras backend as specified in {}'.
                             format(self.config))
                    try:
                        import tensorflow
