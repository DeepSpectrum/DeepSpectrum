...
# ==============================================================================
"""Abstractions for the head(s) of a model. This version just adds UAR as a metric."""
...
from .custom_metrics import streaming_concordance_correlation_coefficient
...
try:
    import tfplot
    tf.logging.info(
        'Found tfplot package. Saving confusion matrix plots to tensorboard for all classifiers.'
    )
except ImportError:
    _TF_PLOT = False
    ...
    with ops.name_scope(
        ...
        if (weights.shape.ndims is not None and logits.shape.ndims is not None
            ...
            assert_dimension = check_ops.assert_equal(
                ...
                message=err_msg,
                data=[
                    'logits_shape: ', logits_shape, 'weights_shape: ',
                    weights_shape
                ])
            with ops.control_dependencies([assert_dimension]):
                return array_ops.expand_dims(weights, -1, name=scope)
        supported_weights_shape = array_ops.concat(
            [logits_shape[:-1], [1]], axis=0)
        if allow_per_logit_weights:
            condition = math_ops.reduce_any([
                math_ops.reduce_all(
                    math_ops.equal(logits_shape, weights_shape)),
                math_ops.reduce_all(
    ...
    for required_arg in ['labels', 'logits']:
        if required_arg not in loss_fn_args:
            raise ValueError('loss_fn must contain argument: {}. '
                             'Given arguments: {}'.format(
                                 required_arg, loss_fn_args))
    invalid_args = list(
        set(loss_fn_args) - set(['labels', 'logits', 'features']))
    if invalid_args:
        raise ValueError(
            'loss_fn has unexpected args: {}'.format(invalid_args))
  ...
  """
    ...
    with ops.name_scope(
        ...
        expected_loss_shape = array_ops.concat(
            [logits_shape[:-1], [expected_loss_dim]],
            axis=0,
        ...
        check_loss_shape_op = control_flow_ops.Assert(
            math_ops.reduce_all(
                math_ops.equal(loss_shape, expected_loss_shape)),
            data=[
...
def _accuracy_baseline(labels_mean):
  ...
  """
    with ops.name_scope(None, 'accuracy_baseline', labels_mean):
        ...
        return (math_ops.maximum(value, 1. - value, name='value'),
...
def _confusion_matrix(labels, predictions, num_classes, name=None):
...
def _precision_at_threshold(labels, predictions, weights, threshold,
    ...
    with ops.name_scope(name, 'precision_at_%s' % threshold,
        ...
        precision_tensor, update_op = metrics_lib.precision_at_thresholds(
            ...
            weights=weights,
            name=scope)
        return array_ops.squeeze(precision_tensor), array_ops.squeeze(
            update_op)
        ...
        with ops.name_scope(
            ...
            metric_ops = {
                ...
                metrics_lib.accuracy(
                    ...
                    weights=weights,
                    name=keys.ACCURACY),
                _summary_key(self._name, keys.UAR):
            ...
            if regularization_loss is not None:
                metric_ops[_summary_key(
                    self._name, keys.LOSS_REGULARIZATION)] = (metrics_lib.mean(
        ...
        with ops.name_scope(self._name, 'head'):
            ...
            with ops.name_scope(None, 'predictions', (logits, )):
                ...
                if self._label_vocabulary:
                    ...
                    classes = string_ops.as_string(
                        class_ids, name='str_classes')
...
def _binary_logistic_head_with_sigmoid_cross_entropy_loss(
  ...
  """
    ...
    if label_vocabulary is not None and not isinstance(label_vocabulary,
                                                       (list, tuple)):
        raise ValueError(
            'label_vocabulary should be a list or tuple. Given type: {}'.
            format(type(label_vocabulary)))
    ...
    def _eval_metric_ops(self, labels, logits, logistic, class_ids, weights,
        ...
        with ops.name_scope(None, 'metrics',
            ...
            metric_ops = {
                ...
                _summary_key(self._name, keys.CONFUSION):
                _confusion_matrix(
                    labels=labels, predictions=class_ids, num_classes=2)
            }
            if regularization_loss is not None:
                metric_ops[_summary_key(
                    self._name, keys.LOSS_REGULARIZATION)] = (metrics_lib.mean(
            ...
            for threshold in self._thresholds:
                ...
                metric_ops[_summary_key(
                    self._name, accuracy_key)] = _accuracy_at_threshold(
                        labels=labels,
                        predictions=logistic,
                        weights=weights,
                        threshold=threshold,
                ...
                metric_ops[_summary_key(
                    self._name, precision_key)] = _precision_at_threshold(
                        labels=labels,
                        predictions=logistic,
                        weights=weights,
                        threshold=threshold,
        ...
        with ops.name_scope(self._name, 'head'):
            ...
            if mode == model_fn.ModeKeys.EVAL:
                ...
                if regularization_loss is not None:
                    ...
                    eval_metric_ops[
                        regularization_loss_key] = metrics_lib.mean(
                            values=regularization_loss,
                            name=keys.LOSS_REGULARIZATION)
                return model_fn.EstimatorSpec(
                    mode=model_fn.ModeKeys.EVAL,
