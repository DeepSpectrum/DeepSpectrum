...
def main():
    configuration = Configuration(plotting=False, file_type='png', parser_description=DESCRIPTION_IMAGE_FEATURES)
    configuration.parse_arguments()
    plots = image_reader(configuration.files)
    print('Loading model and weights...')
    if configuration.backend == 'caffe':
        from src.backend.extractor import CaffeExtractor
        extractor = CaffeExtractor(
            images=plots, **configuration.extraction_args)
    elif configuration.backend == 'tensorflow':
        from src.backend.extractor import TensorFlowExtractor
