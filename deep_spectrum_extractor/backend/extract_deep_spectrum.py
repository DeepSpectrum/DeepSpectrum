...
# def extract_features_from_image_blob(img_blob, session, net, layer='fc7'):
#     """
#     Extracts deep features from a given image blob using a pre-loaded Caffe Network. Features from high dimenional
#     layers are flattened to 1D.
#     :param img_blob: blob containing image data in .png format
...
#         img = imread_from_blob(img_blob, 'png')
#         img = img[:,:,:-1]
#         img = models.process_image(img, data_spec)
#     except IOError:
#         print('Error while reading the spectrogram blob.')
#         return None
#     image = session.run([img])
#     features = session.run([net.layers[layer]], feed_dict={net.inputs['data']: image})
#     return np.ravel(features)


# def extract_features_from_wav(wav_file, session, net, nfft=256, layer='fc7', cmap='viridis', size=227, chunksize=None,
#                               step=None, output_spectrograms=None, y_limit=None, start=0, end=None):
#     """
...
#     :param wav_file: path to an existing .wav file
#     :param session: configured transformer object belonging to a loaded Caffe Network
#     :param net: a pre loaded Caffe Network used for feature extraction
...
#         if chunksize:
#             yield start + (index * step), extract_features_from_image_blob(img_blob, session, net,
#                                                                            layer=layer)
#         else:
#             yield None, extract_features_from_image_blob(img_blob, session, net, layer=layer)

# def plot_spectrograms_from_wav(wav_file, nfft=256, layer='fc7', cmap='viridis', size=227, chunksize=None,
#                               step=None, output_spectrograms=None, y_limit=None, start=0, end=None, net=None):
#     """
#     Extracts deep spectrum features from a given wav-file using either the whole file or equally sized chunks as basis
#     for the spectrogram plots.
#     :param wav_file: path to an existing .wav file
