...
#         spectrogram_directory = join(config.output_spectrograms,
#                                      get_relative_path(file, config.input))
#         makedirs(spectrogram_directory, exist_ok=True)
#     if config.output_wavs:
#         wav_directory = join(config.output_wavs,
#                              get_relative_path(file, config.input))
#         makedirs(wav_directory, exist_ok=True)
#     return np.asarray([
#         plot
#         for plot in eds.plot(
#             file,
#             output_folder=spectrogram_directory,
#             wav_folder=wav_directory,
#             **kwargs)
#     ])
...
#                   continuous_labels, window, hop, start, no_timestamps,
#                   no_labels):
...
#                 zip(filenames, features), total=len(features)):
#             if no_labels:
#                 classes = None
#             else:
#                 classes = [(class_name, '{' + ','.join(class_type) + '}')
#                            if class_type else (class_name, 'numeric')
#                            for class_name, class_type in labels]

#             file_name = basename(file_name)
#             for idx, feature_vector in enumerate(features):
#                 if not writer:
#                     attributes = _determine_attributes(write_timestamp,
#                                                        feature_vector, classes)
...
#                     row = [file_name] + [str(timestamp)] + list(
#                         map(str, feature_vector))
#                     if not no_labels:
#                         row += labels
#                 else:
